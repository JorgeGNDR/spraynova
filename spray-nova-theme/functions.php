...
function spray_nova_woocommerce_integration() {
	...
	add_action( 'woocommerce_archive_description', 'spray_nova_shop_intro', 5 );
...
/**
 * Keep the shop archive title useful and predictable without affecting
 * product category, brand or attribute archive titles.
 *
 * @param string $page_title Archive title generated by WooCommerce.
 * @return string
 */
function spray_nova_shop_page_title( $page_title ) {
	return function_exists( 'is_shop' ) && is_shop()
		? __( 'TIENDA', 'spray-nova' )
		: $page_title;
}
add_filter( 'woocommerce_page_title', 'spray_nova_shop_page_title' );

/**
 * Add the requested concise shop introduction below its H1. Taxonomy
 * descriptions remain managed by WooCommerce, so editors can change them
 * from Products > Categories or Products > Brands.
 */
function spray_nova_shop_intro() {
	if ( ! function_exists( 'is_shop' ) || ! is_shop() || is_search() || 0 !== absint( get_query_var( 'paged' ) ) ) {
		return;
	}
...
/**
 * Read a technical value from native WooCommerce attributes. Both global
 * attributes (pa_*) and product-specific attributes are supported.
 *
 * @param WC_Product $product    Product object.
 * @param string[]   $candidates Attribute slugs or names to try.
 * @return string
 */
function spray_nova_product_attribute_value( $product, $candidates ) {
	if ( ! $product instanceof WC_Product ) {
		return '';
	}
	...
	$normalized_candidates = array_map(
		static function( $candidate ) {
			return sanitize_title( preg_replace( '/^pa_/', '', $candidate ) );
		},
		$candidates
	);

	foreach ( $candidates as $candidate ) {
		$value = trim( (string) $product->get_attribute( $candidate ) );
		if ( '' !== $value ) {
			return $value;
		}
	}
	...
	foreach ( $product->get_attributes() as $attribute ) {
		...
		$normalized = sanitize_title( preg_replace( '/^pa_/', '', $name ) );

		if ( in_array( $normalized, $normalized_candidates, true ) ) {
...
/**
 * Return the native WooCommerce brand as a crawlable archive link.
 *
 * @param WC_Product $product Product object.
 * @return string Safe HTML or an empty string.
 */
function spray_nova_product_brand_value( $product ) {
	if ( ! $product instanceof WC_Product ) {
		return '';
	}
	...
	if ( taxonomy_exists( 'product_brand' ) ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				$link = get_term_link( $term );
				if ( ! is_wp_error( $link ) ) {
					$links[] = sprintf( '<a href="%1$s">%2$s</a>', esc_url( $link ), esc_html( $term->name ) );
				}
			}
	...
	$attribute_brand = spray_nova_product_attribute_value( $product, array( 'pa_marca', 'marca', 'brand' ) );
	return $attribute_brand ? esc_html( $attribute_brand ) : '';
}
...
function spray_nova_spray_color_selector() {
	...
	$brand    = spray_nova_product_brand_value( $product );
	$format   = spray_nova_product_attribute_value( $product, array( 'pa_formato', 'pa_capacidad', 'formato', 'capacidad' ) );
	$pressure = spray_nova_product_attribute_value( $product, array( 'pa_presion', 'presion', 'pressure' ) );
	$finish   = spray_nova_product_attribute_value( $product, array( 'pa_acabado', 'acabado', 'finish' ) );

	// Preserve existing products while their legacy fields are migrated to WooCommerce attributes.
	$brand    = $brand ?: esc_html( get_post_meta( $product->get_id(), '_spray_nova_brand', true ) );
	$format   = $format ?: get_post_meta( $product->get_id(), '_spray_nova_format', true );
	$pressure = $pressure ?: get_post_meta( $product->get_id(), '_spray_nova_pressure', true );
	$finish   = $finish ?: get_post_meta( $product->get_id(), '_spray_nova_finish', true );

	$specs = array_filter( array(
		__( 'Marca', 'spray-nova' )   => $brand,
		__( 'Formato', 'spray-nova' ) => $format,
		__( 'Presión', 'spray-nova' ) => $pressure,
		__( 'Acabado', 'spray-nova' ) => $finish,
	) );
		...
		<?php if ( $specs ) : ?>
			<div class="spray-product-specs" aria-label="<?php esc_attr_e( 'Características del spray', 'spray-nova' ); ?>">
				<?php foreach ( $specs as $label => $value ) : ?>
					<span><strong><?php echo esc_html( $label ); ?></strong><?php echo wp_kses_post( $value ); ?></span>
				<?php endforeach; ?>
			</div>
		<?php endif; ?>
			...
			<?php foreach ( $colors as $color ) : ?>
				...
				>
					...
					<div class="spray-qty" aria-label="<?php esc_attr_e( 'Cantidad', 'spray-nova' ); ?>">
						<button type="button" class="spray-qty-minus" aria-label="<?php esc_attr_e( 'Reducir cantidad', 'spray-nova' ); ?>" <?php disabled( ! $color['is_enabled'] ); ?>>−</button>
						<input type="number" min="0" step="1" value="0" inputmode="numeric" <?php disabled( ! $color['is_enabled'] ); ?>>
						<button type="button" class="spray-qty-plus" aria-label="<?php esc_attr_e( 'Aumentar cantidad', 'spray-nova' ); ?>" <?php disabled( ! $color['is_enabled'] ); ?>>+</button>
					</div>
				</article>
			<?php endforeach; ?>
		</div>
